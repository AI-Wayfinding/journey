...
| `POST /v1/auth/passkey/login/options`, `/login/verify` | Request user verification and the account's PRF result in the same passkey tap. A verified session can request options again to unlock a new tab. |
| `POST /v1/auth/logout` | Revoke the cookie. |
| `PUT /v1/me/keys`, `GET /v1/me/keys` | A passkey-verified session updates or reads only `{version:1,identity,signing}` AES-GCM ciphertext. Old formats are rejected on PUT and hidden on GET. No plaintext private keys. No-store responses. |

...
| `POST /v1/auth/passkey/register/options`, `/register/verify` | Request a resident passkey with user verification and a PRF result. Verify accepts `{response,sealed:{version:1,identity,signing}}` and saves one credential and its encrypted keys together. No PRF support or invalid sealed keys means nothing is saved. |
| `POST /v1/auth/passkey/login/options`, `/login/verify` | With an email-link session, request user verification and the account's PRF result in one passkey tap. Legacy accounts receive their old input and the app-wide input for key migration. A verified session can request options again to unlock a new tab. |
| `POST /v1/auth/passkey/start`, `/finish` | Sign in without an email link: discoverable passkey options (no credential allow-list), a five-minute pre-session challenge cookie, and a verified 12-hour session after assertion. An account missing its stored email or still using the legacy PRF input must use an email link instead. |
| `POST /v1/auth/logout` | Revoke the cookie. |
| `PUT /v1/me/keys`, `GET /v1/me/keys` | A passkey-verified session updates or reads only `{version:1,identity,signing}` AES-GCM ciphertext. A legacy account can include `migrate:true` after email-link passkey verification to atomically replace its ciphertext and clear its old salt. Old formats are rejected on PUT and hidden on GET. No plaintext private keys. No-store responses. |
| `POST /v1/journeys` | Create with `{id,name,creator:{id,recipient,signingKey},genesis,wraps,recoveryWrap,minClientVersion}`. The client generates `id` before encrypting genesis. The creator email comes from the verified sign-in session. Each wrap is `{principal,epoch,wrap}`. |
...
| `POST /v1/journeys/:id/invites` | `{inviteIdHash,expiresAt,support?,email?,inviteId?}`: create a one-use invite that expires within seven days. With `email`, the request also carries the matching client-generated secret `inviteId` so the Worker can email the link; the address is not stored. `support:true` marks a Wayfinding support invitation; admission must be read-only and expires when the invite expires. |
...
Every state-changing request needs `Origin: https://app.wayfinding.support` and `X-Wayfinding: 1`. A person sends the httpOnly, Secure, SameSite=Strict `wayfinding_session` cookie and `X-Principal` on journey routes. Magic-link start always follows the same account-independent issuance path and returns 202, whether the account exists or not. Email addresses are normalized and hashed with HMAC-SHA-256 under `EMAIL_HASH_KEY`. A verified email session can register or request a login assertion, but cannot access journeys until passkey verification succeeds. WebAuthn checks `RP_ID`, `ORIGIN`, required user verification, a single-use challenge, and the credential counter. Login options include `extensions.prf.eval.first`: a stable app-wide 32-byte input derived from `EMAIL_HASH_KEY`, not a new deployment secret. An account still holding an older per-account input gets both that input and the app-wide input after email-link sign-in. Its device decrypts with the old PRF result, reseals with the second result, then updates the versioned ciphertext and clears the old input in one transaction. It cannot use email-free sign-in until then; its keys and journeys remain intact. A verified session can request options again to unlock keys in another tab with one tap. The server stores a single credential public key, counter, transports and encrypted keys; it never receives a PRF output. On sign-up, registration and sealed keys are saved in one SQLite transaction, so a failed attempt cannot leave a half-saved credential. A client reports whether PRF was enabled; the server cannot independently prove that a browser returned a usable PRF output, so the client must test it before submitting its encrypted keys.

The schema version 4 migration drops the former two-passkey credentials and sealed keys and invalidates prior passkey-verified sessions. People who signed up before this change must use a fresh email link and sign up again. There are no real production journeys to convert; old journey keys cannot be recovered through this migration.

The holder makes 32 random bytes, puts them after `#` in the invitation URL, and posts their base64url SHA-256 digest as `inviteIdHash`. For email invitations only, the link secret passes through the Worker so it can email the link via `MAGIC_EMAIL` from `noreply@wayfinding.support`, with `hello@wayfinding.support` as reply-to; neither the email address nor the secret is saved in invitation storage. The fragment never goes to the server when the recipient opens the link. Invitations sent by email are limited per journey and account. The invitee submits the random value as `inviteId`, proves email control and registers a passkey. The server consumes the invite once and holds the new principal as pending. A holder's device verifies the signed log, posts an encrypted `member.add` entry with an access addition and `memberWraps`, then the new principal can read. A support invitation produces a signed support marker and read-only, expiring membership; the Worker rejects any writable or extended support access change, and the signed-log verifier refuses management grants to support members. `inviteSecretProof` is reserved for a later proof design; this version checks the hash preimage supplied as `inviteId`.
...
Plain storage: journey ID and name, creator email in the creator's registry row only (also held with the sign-in token for 15 minutes and the session for 12 hours, so journey creation needs no retyping), record metadata and size, timestamps, member count, storage estimate, minimum client version, account email *hashes* and each account's plain email address (stored at sign-in so a returning person can continue with a passkey alone; not shown in the admin view), credential public keys, legacy per-account PRF inputs pending migration, opaque principal IDs with kind/scope/expiry and support invitation marker, an opaque link from an approved agent to its approving person (needed to remove their agents together), and pending invite public keys. The operator can see which email addresses are members of which journeys; request timing can link active accounts. Email delivery exposes destinations to the mail provider.
